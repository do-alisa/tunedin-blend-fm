...
function App() {
  ...
  async function exportToSpotify() {
    try {
      ...
      const res = await fetch("http://127.0.0.1:3001/api/spotify/export", {
        ...
        body: JSON.stringify({
          name: `TunedIn Blend: ${user1} + ${user2}`,
  ...
  return (
    <div style={{ padding: "40px", fontFamily: "sans-serif", maxWidth: 800 }}>
      <h1>TunedIn Blend</h1>
      ...
      <button
        ...
        onClick={async () => {
          try {
            ...
            // If profiles don't exist yet, fetch them
            if (!p1 || !p2) {
              ...
              const profiles = await Promise.all([
                fetchTasteProfile(user1.trim()),
                fetchTasteProfile(user2.trim()),
              ]);
      ...
      {blend.length > 0 && (
        <>
          <h2 style={{ marginTop: 24 }}>Blend</h2>
          <ol>
            {blend.map((t, idx) => (
              <li key={`${t.artist}-${t.title}-${idx}`}>
